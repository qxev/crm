...
<div class="content">
<s:form id="projectListForm" action="/client/contractProject!otherContractProjectList.action">
	<s:hidden name="businessTypeId" id="businessTypeId" value="%{businessTypeId}" />
	...
	<table align="center">
		<tr>
			...
			<td>PM</td><td><s:select list="searchView.pms" headerKey="0" headerValue="全部" name="searchView.pmId" listKey="id" listValue="name" style="width:140px"/></td>
	...
	<table class="report" borderColor="#f2f2f2" border="1">
		...
		<tr>
			...
			<report:pages action="${ctx}/client/contractProject!otherContractProjectList.action" excel="${ctx}/client/contractProject!excelOutput.action" page="${page}" businessTypeId="${businessTypeId}"/>
			</td>
		</tr>
		<tr align="center">
			<td></td>
		...
		<tr>
			<th width="5%">序号</th>
			<th width="25%"><report:tableTitle title="客户名" action="contractProject!otherContractProjectList.action" sqlName="contractClient.name" page="${page}" ctx="${ctx}" businessTypeId="${businessTypeId}"/></th>
			<th width="15%"><report:tableTitle title="项目名" action="contractProject!otherContractProjectList.action" sqlName="name" page="${page}" ctx="${ctx}" businessTypeId="${businessTypeId}"/></th>
			<th width="10%">PM</th>
			<th width="10%">BD管理员</th>
			<th width="10%"><report:tableTitle title="行业" action="contractProject!otherContractProjectList.action" sqlName="industry.name" page="${page}" ctx="${ctx}" businessTypeId="${businessTypeId}"/></th>
			<th width="10%"><report:tableTitle title="客户来源" action="contractProject!otherContractProjectList.action" sqlName="contractClient.type" page="${page}" ctx="${ctx}" businessTypeId="${businessTypeId}"/></th>
			<th width="20%">操作</th>
		</tr>
