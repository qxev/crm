...
<s:form id="projectListForm" action="/client/otherProjectList.action">
	...
	<table align="center">
		<tr>
			...
			<td width="48%">分析师选择: <s:select list="searchView.pms" headerKey="0" headerValue="全部" name="searchView.pmId" listKey="id" listValue="name"/>
	...
	<table class="report" borderColor="#f2f2f2" border="1">
		...
		<s:else>
			<tr class="table_style2">
				...
				<c:if test="${(sessionScope.role['type'] == '2')||(sessionScope.role['type'] == '3')}"><a href="${ctx}/template/${bsType}.doc">模板下载</a>
				<a href="${ctx}/client/contract!saveContract.action?projectId=${id}">创建合同</a>
				</c:if></td>
			</tr>
		</s:iterator>
		<tr>
