...
public class ServiceFeeRateDao extends HibernateDaoSupport {
	...
	public List<ServiceFeeRate> getAll(Page<ServiceFeeRate> page,boolean hasTotal) {
		...
		sb.append(" select {se.*} from service_fee_rate {se} ");
		...
		if (!(pars.length==7 && "false".equals(pars[6]))){
			sb.append(" join (select max(start_date) d,account_id from service_fee_rate group by account_id) q ");	
	...
	@SuppressWarnings("unchecked")
	public ServiceFeeRate getServiceFeeRateByDate(Date date, Integer accountId, Integer type) {
		Object[] obj = { date, date, accountId, type };
		List<ServiceFeeRate> list = (List<ServiceFeeRate>)this.getHibernateTemplate().find(
				" from ServiceFeeRate where startDate <= ? and endDate >= ? and account.id = ? and type = ? ", obj);
		if (list != null && list.size() > 0) {
			return (ServiceFeeRate) list.get(0);
		}
		return null;
	}
	
	private void setQuery(Page<ServiceFeeRate> page, String[] pars,
	...
	public int getTotalCount(Page<ServiceFeeRate> page) {
		...
		StringBuffer sb = new StringBuffer();
		if (!(pars.length==7 && "false".equals(pars[6]))){
			sb.append(" select count(distinct account.id) from ServiceFeeRate where deleted = 0  ");
		} else {
			sb.append(" select count(*) from ServiceFeeRate where deleted = 0  ");
		}
	...
	public int getCountByBetweenDay(String startDate, String endDate, Integer id, Integer accountId, Integer type) {
		StringBuffer sb = new StringBuffer();
		sb.append(" select count(*) from ServiceFeeRate where deleted = 0 and account.id = :accountId and type = :type and ((:startDate between startDate and endDate) or (:endDate between startDate and endDate)) ");
		if (id!=0){
			sb.append(" and id != :id");
		}
		Query query = getSession().createQuery(sb.toString());
	...
	public void save(ServiceFeeRate serviceFeeRate) {
		this.getHibernateTemplate().saveOrUpdate(serviceFeeRate);
	}
	
	public void delete(ServiceFeeRate serviceFeeRate) {
		this.getHibernateTemplate().delete(serviceFeeRate);
	}
	
	public ServiceFeeRate createServiceFeeRate() {
		ServiceFeeRate serviceFeeRate = new ServiceFeeRate();
		serviceFeeRate.setCreateAt(new Date());
		return serviceFeeRate;
	}

	public ServiceFeeRate getServiceFeeRateById(Integer id) {
		return (ServiceFeeRate)this.getHibernateTemplate().get(ServiceFeeRate.class, id);
	}
}
