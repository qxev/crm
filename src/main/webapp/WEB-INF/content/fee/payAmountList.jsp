...
<s:form id="payAmountListForm" action="/fee/payAmountList.action">
<div class="content">
	...
	<table align="center">
		...
		<tr>
			...
			<td><input type="image" src="${ctx}/images/search.gif" onClick="userSubmit('payAmountListForm')"/></td>
		</tr>
	</table>
	</div>
	<table class="report" borderColor="#f2f2f2" border="1">
		<tr>
			<td align="center" colspan="9" width="80%" class="green-bg1" colspan="5"><h1>达闻付款历史</h1></td>
		</tr>
		<tr>
			<td align="right" colspan="9">
				<report:pages action="payAmountList.action" excel="payAmountList!excelOutput.action" page="${page}"/>
			</td>
		</tr>
		<tr align="center">
			<td></td>
		...
		<tr>
			<th width="5%">序号</th>
			<th width="18%"><report:tableTitle title="客户名" action="payAmountList.action" sqlName="s.account.project.client.name" page="${page}" ctx="${ctx}"/></th>
			<th width="10%"><report:tableTitle title="项目名" action="payAmountList.action" sqlName="s.account.project.name" page="${page}" ctx="${ctx}"/></th>
			<th width="18%"><report:tableTitle title="账户名" action="payAmountList.action" sqlName="s.account.name" page="${page}" ctx="${ctx}"/></th>
			<th width="8%"><report:tableTitle title="媒体名" action="payAmountList.action" sqlName="s.account.channel.media.name" page="${page}" ctx="${ctx}"/></th>
			<th width="18%"><report:tableTitle title="渠道名" action="payAmountList.action" sqlName="s.account.channel.name" page="${page}" ctx="${ctx}"/></th>
			<th width="18%"><report:tableTitle title="冲值日期" action="payAmountList.action" sqlName="s.supplementDate" page="${page}" ctx="${ctx}"/></th>
			<th width="18%"><report:tableTitle title="金额" action="payAmountList.action" sqlName="payAmount" page="${page}" ctx="${ctx}"/></th>
			<th width="10%">操作</th>
		</tr>
		...
		<s:else>
			<tr class="table_style2">
				...
				<td align="right"><report:bigDecimalFormat value = "${payAmount}" />&nbsp;</td>
