...
<div class="content">
<s:form id="serviceFeeForm" action="/report/serviceFeeList.action">
	...
	<table align="center">
		...
		<tr>
			...
			<td>是否显示最近记录<s:checkbox name="searchView.repeat"/><input type="image" src="${ctx}/images/search.gif" onClick="userSubmit('serviceFeeForm')"/></td>
		</tr>
	</table>
	</div>
	<table class="report" borderColor="#f2f2f2" border="1">
		<tr>
			<td align="center" colspan="9" width="80%" class="green-bg1" colspan="5"><h1>服务费历史</h1></td>
		</tr>
		<tr>
			<td align="right" colspan="9">
				<report:pages action="serviceFee.action" excel="serviceFeeList!excelOutput.action" page="${page}"/>
			</td>
		</tr>
		<tr align="center">
			<td></td>
		...
		<tr>
			<th width="5%">序号</th>
			<th width="21%"><report:tableTitle title="客户名" action="serviceFeeList.action" sqlName="c.name" page="${page}" ctx="${ctx}"/></th>
			<th width="10%"><report:tableTitle title="项目名" action="serviceFeeList.action" sqlName="p.name" page="${page}" ctx="${ctx}"/></th>
			<th width="11%"><report:tableTitle title="账户名" action="serviceFeeList.action" sqlName="a.name" page="${page}" ctx="${ctx}"/></th>
			<th width="11%"><report:tableTitle title="调整日期" action="serviceFeeList.action" sqlName="se.create_at" page="${page}" ctx="${ctx}"/></th>
			<th width="11%"><report:tableTitle title="调整前服务费" action="serviceFeeList.action" sqlName="se.pre_value" page="${page}" ctx="${ctx}"/></th>
			<th width="11%"><report:tableTitle title="调整后服务费" action="serviceFeeList.action" sqlName="se.value" page="${page}" ctx="${ctx}"/></th>
			<th width="11%"><report:tableTitle title="区间开始日期" action="serviceFeeList.action" sqlName="se.start_date" page="${page}" ctx="${ctx}"/></th>
			<th width="11%"><report:tableTitle title="区间结束日期" action="serviceFeeList.action" sqlName="se.end_date" page="${page}" ctx="${ctx}"/></th>
		</tr>
